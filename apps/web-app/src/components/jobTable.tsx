        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          <Select value={locationFilter} onValueChange={setLocationFilter}>
            ...
            <SelectContent>
              ...
              {Array.from(new Set(jobs.map((job) => job.location).filter(Boolean))).map((location) => (
          ...
          <Select value={jobTypeFilter} onValueChange={setJobTypeFilter}>
            <SelectTrigger className="w-full sm:w-[180px]"><SelectValue placeholder="Job Type" /></SelectTrigger>
            <SelectContent>
              <SelectItem value="all">All Types</SelectItem>
              {Array.from(new Set(jobs.map((job) => job.jobType).filter(Boolean))).map((type) => (
        ...
        <div className="overflow-x-auto">
          <Table>
            <TableHeader>
              <TableRow>
                <SortableHead column="title" label={JOB.JOB_TABLE.TITLE} currentSortBy={sortBy} currentSortOrder={sortOrder} onSort={setSort} />
                <TableHead className="text-left text-sm font-medium">{JOB.JOB_TABLE.LOCATION}</TableHead>
                <TableHead className="text-left text-sm font-medium">{JOB.JOB_TABLE.JOB_TYPE}</TableHead>
                <th className="px-4 py-2 text-left text-sm font-medium">{JOB.JOB_TABLE.OPENINGS}</th>
                <SortableHead column="createdAt" label={JOB.JOB_TABLE.POSTED_AT} currentSortBy={sortBy} currentSortOrder={sortOrder} onSort={setSort} />
                <th></th>
              </TableRow>
            </TableHeader>
            <TableBody>
              {jobs.filter((job) => {
                const locationOk = locationFilter === "all" || job.location === locationFilter;
                const typeOk = jobTypeFilter === "all" || job.jobType === jobTypeFilter;
