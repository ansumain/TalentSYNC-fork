...
import { fileParamsSchema } from './validations/resume.validation';
...
app.get('/files/:filename', authenticationMiddleware, validateRequest({ params: fileParamsSchema }), async (req: Request, res: Response, next: NextFunction) => {
    const filename = path.basename(req.params.filename as string);
    if (!filename) {
        next(badRequestError('Invalid filename', 'INVALID_FILENAME'));
        return;
